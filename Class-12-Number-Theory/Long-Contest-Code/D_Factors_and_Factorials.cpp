#include <bits/stdc++.h>
using namespace std;

#define endn                '\n'
#define umap                unordered_map
#define uset                unordered_set
#define pb                  push_back

typedef short int               sint;
typedef unsigned int            uint;
typedef long long int           llint;
typedef unsigned long long int  ullint;

#define debug(...) __f(#__VA_ARGS__, __VA_ARGS__)
template < typename Arg1 >void __f(const char* name, Arg1&& arg1){cout << name << " = " << arg1 << std::endl;}
template < typename Arg1, typename... Args> void __f(const char* names, Arg1&& arg1, Args&&... args){ const char* comma = strchr(names+1, ','); cout.write(names, comma - names) << " = " << arg1 <<" | "; __f(comma+1, args...);}
inline void first_io() {ios_base::sync_with_stdio(false);cin.tie(NULL);}
inline void file_io() {freopen("input.txt", "r", stdin);freopen("output.txt", "w", stdout);}
vector<int> get_randoms(int amount, int first, int last) {vector<int> tmp;srand(time(0));for (int i = 0; i < amount; i++)tmp.push_back(first + ((rand() % (last - first + 1))));return tmp;}
template <typename Tp> void print(Tp arr[], int n) {for (int i = 0; i < n; i++) cout << arr[i] << " "; cout << '\n';}
template <typename Tp> void print(const Tp &vc) {for (auto &ith : vc) cout << ith << " "; cout << '\n';}
template <typename Tp> void print(stack<Tp> st) {while (!st.empty()) { cout << st.top() << " "; st.pop();} cout << '\n';}
template <typename Tp, typename Tp2> void print(unordered_map<Tp, Tp2> ump) {for (auto &ith : ump) {cout << ith.first << " " << ith.second << '\n';} cout << '\n';}

#define GLOBAL_ARRAY_SIZE   10000005
#define LOCAL_ARRAY_SIZE    100005
#define PI                  acos(-1.0)
#define EPS                 1e-9

#define mx 110
bitset<mx> mark;
bool primes[mx];
void sieve() {
    primes[2] = true;
    int i, j;
    for (i = 3; i <= sqrt(int(mx)); i += 2) {
        if (mark[i] == false) {
            primes[i] = true;
            for (j = i*i; j <= mx; j += i*2) {
                mark[j] = true;

            }
        }
    }
    for (; i <= mx; i += 2) {
        if (mark[i] == false) {
            primes[i] = true;
        }
    }
}


int main(void) {
    sieve();

    int t;
    while (scanf("%d", &t) == 1 && t) {
        int table[100] = {0}, mx2 = 0;

        for (int nth = 2; nth <= t; nth++) {
            int n = nth;
            int root = sqrt(n);
            for (int i = 2; i <= root; i++) {
                while (n % i == 0) {
                    table[i]++;
                    mx2 = max(mx2, i);
                    n /= i;
                }
                root = sqrt(n);
            }
            if (n > 1) {
                table[n]++;
                mx2 = max(mx2, n);
            }
        }

        // print(table, 100);
        int len = 99;
        while (table[len] == 0) len--;
        printf("%3d! =", t);

        int ct = 0, i;
        for (i = 2; i <= len && ct < 15; i++) {
            if (primes[i]) {
                printf("%3d", table[i]);
                ct++;
            }
        }
        if (i <= len) printf("\n      ");
        while (i <= len) {
            if (primes[i]) {
                printf("%3d", table[i]);
            }
            i++;
        }
        printf("\n");
    }

    return 0;
}
// Solved by: Shihab Mahamud (github.com/shihab4t)
// Thursday, October 14, 2021 | 06:05:44 AM (+06)
// Accepted: 10:02:19 AM (+06)


// Another Solutions (first on)
/*
#include <bits/stdc++.h>
using namespace std;

#define endn                '\n'
#define umap                unordered_map
#define uset                unordered_set
#define pb                  push_back

typedef short int               sint;
typedef unsigned int            uint;
typedef long long int           llint;
typedef unsigned long long int  ullint;

#define debug(...) __f(#__VA_ARGS__, __VA_ARGS__)
template < typename Arg1 >void __f(const char* name, Arg1&& arg1){cout << name << " = " << arg1 << std::endl;}
template < typename Arg1, typename... Args> void __f(const char* names, Arg1&& arg1, Args&&... args){ const char* comma = strchr(names+1, ','); cout.write(names, comma - names) << " = " << arg1 <<" | "; __f(comma+1, args...);}
inline void first_io() {ios_base::sync_with_stdio(false);cin.tie(NULL);}
inline void file_io() {freopen("input.txt", "r", stdin);freopen("output.txt", "w", stdout);}
vector<int> get_randoms(int amount, int first, int last) {vector<int> tmp;srand(time(0));for (int i = 0; i < amount; i++)tmp.push_back(first + ((rand() % (last - first + 1))));return tmp;}
template <typename Tp> void print(Tp arr[], int n) {for (int i = 0; i < n; i++) cout << arr[i] << " "; cout << '\n';}
template <typename Tp> void print(const Tp &vc) {for (auto &ith : vc) cout << ith << " "; cout << '\n';}
template <typename Tp> void print(stack<Tp> st) {while (!st.empty()) { cout << st.top() << " "; st.pop();} cout << '\n';}
template <typename Tp, typename Tp2> void print(unordered_map<Tp, Tp2> ump) {for (auto &ith : ump) {cout << ith.first << " " << ith.second << '\n';} cout << '\n';}

#define GLOBAL_ARRAY_SIZE   10000005
#define LOCAL_ARRAY_SIZE    100005
#define PI                  acos(-1.0)
#define EPS                 1e-9


#define mx 250
bitset<mx> mark;
bool primes[mx];
void sieve() {
    // primes.push_back(2);
    primes[2] = true;
    int i, j;
    for (i = 3; i <= sqrt(int(mx)); i += 2) {
        if (mark[i] == false) {
            // primes.push_back(i);
            primes[i] = true;
            for (j = i*i; j <= mx; j += i*2) {
                mark[j] = true;

            }
        }
    }
    for (; i <= mx; i += 2) {
        if (mark[i] == false) {
            // primes.push_back(i);
            primes[i] = true;
        }
    }
}

vector <int> prime_factors(int n) {
    vector<int> rs;
    int root = sqrt(n);
    for (int i = 2; i <= root; i++) {
        while (n % i == 0) {
            rs.push_back(i);
            n /= i;
        }
        root = sqrt(n);
    }
    if (n > 1) rs.push_back(n);
    return rs;
    // O(sqrt(n))
}


int main(void) {
    // first_io();
    // file_io();

    sieve();

    int n, len, k, i, j;
    while (true) {
        scanf("%d", &n);
        if (n == 0) return 0;
        printf("%3d! =", n);

        int ct_prime[250];
        memset(ct_prime, 0, sizeof(ct_prime));

        int mx2 = 0;

        for (k = 2; k <= n; k++) {
            auto rs = prime_factors(k);
            for (auto ith : rs) {
                ct_prime[ith]++;
                mx2 = max(ith, mx2);
            }
        }

        j = 2, i = 0;


        while(i < 15 && j <= mx2) {
            if (primes[j] == true) {
                printf("%3d",ct_prime[j]);
                j++;
                i++;
            }
            else {
                j++;
            }
        }
        printf("\n");


        while (j <= mx2) {
            i = 0;
            printf("      ");
            while(i < 15 && j <= mx2) {
                if (primes[j] == true) {
                    printf("%3d",ct_prime[j]);
                    j++;
                    i++;
                }
                else {
                    j++;
                }
            }
            printf("\n");
        }
    }

    return 0;
}
// Solved by: Shihab Mahamud (github.com/shihab4t)
// Thursday, October 14, 2021 | 06:05:44 AM (+06)
*/